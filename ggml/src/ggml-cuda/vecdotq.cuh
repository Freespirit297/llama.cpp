...
#pragma unroll
    for (int i = 0; i < QR4_K; ++i) {
        ...
        sumf_m += ds8[i].x * (dot2 * m[i]);  // multiply constant part of q4_K with sum of q8_1 values
#else
        // no dp4a: use q8_1 block sum instead, 4 threads share one q8_1 block
        sumf_m += 0.25f * ds8[i].y * m[i];
#endif // defined(GGML_USE_HIP) || defined(GGML_USE_MUSA) || __CUDA_ARCH__ >= GGML_CUDA_CC_DP4A
    }
...
static __device__ __forceinline__ float vec_dot_q5_K_q8_1_impl_vmmq(
    ...
    const uint8_t * __restrict__ m, const half2 & dm5, const float2 * __restrict__ ds8) {
...
#pragma unroll
    ...
    return vec_dot_q5_K_q8_1_impl_vmmq(vl, vh, u, sc, m, bq5_K->dm, ds8);
}
